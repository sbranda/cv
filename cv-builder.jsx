...
// DENSITY_LEGACY_SCALE queda solo para convertir perfiles guardados con el
// sistema viejo de 4 botones fijos (textoCompacto/compacta/normal/amplia) al
// nuevo control deslizante, que guarda un número (70-130) directamente.
const DENSITY_LEGACY_SCALE = { textoCompacto: 80, compacta: 90, normal: 100, amplia: 110 };
const getDensityValue = (densidad) => {
  if (typeof densidad === "number") return densidad;
  return DENSITY_LEGACY_SCALE[densidad] ?? 100;
};
...
const TEMPLATES = [
  ...
  },
  {
    id: "bandera",
    name: "Bandera",
    desc: "Franja superior de color con foto que se sobrepone",
    category: "formal",
  },
  {
    id: "banderaCentral",
    name: "Bandera Central",
    desc: "Igual que Bandera, pero foto y nombre centrados",
    category: "formal",
  },
  {
    id: "banderaLinea",
    name: "Bandera Línea",
    desc: "Versión sin bloque de color, con una línea fina de acento",
    category: "creativo",
  },
  {
    id: "barraLateral",
    name: "Barra Lateral",
    desc: "Barra oscura fija con foto y formación, franja de color arriba del nombre",
    category: "fondos",
  },
  {
    id: "perfilBanda",
    name: "Perfil Banda",
    desc: "Franja de color arriba con foto y perfil, dos columnas debajo",
    category: "formal",
  },
  {
    id: "sidebarDiagonal",
    name: "Sidebar Diagonal",
    desc: "Encabezado oscuro con corte diagonal, contacto en columna angosta",
    category: "creativo",
  },
  {
    id: "sidebarPerfil",
    name: "Sidebar Perfil",
    desc: "Barra lateral de color con foto, perfil y contacto centrados",
    category: "fondos",
  },
  {
    id: "bannerAncho",
    name: "Banner Ancho",
    desc: "Encabezado ancho con foto a la derecha, todo el contenido en una columna",
    category: "formal",
  },
  {
    id: "geometrica",
    name: "Geométrica",
    desc: "Forma triangular de color enmarcando la foto, nombre centrado debajo",
    category: "creativo",
  },
  {
    id: "serifDivisor",
    name: "Serif Divisor",
    desc: "Tipografía elegante y línea divisoria vertical entre columna e info",
    category: "formal",
  },
  {
    id: "bandaCeleste",
    name: "Banda Celeste",
    desc: "Franja de color suave arriba, columna izquierda con tinte",
    category: "fondos",
  },
  {
    id: "sidebarReferencias",
    name: "Sidebar Referencias",
    desc: "Barra lateral de color angosta con foto, contenido a la derecha",
    category: "fondos",
  },
];
...
async function callClaude(prompt, imageData) {
...
function SectionOrderList({ order, accent, onReorder }) {
  ...
  return (
    <div role="list" aria-label="Orden de las secciones del CV">
      {order.map((key, i) => (
        <div
          key={key}
          role="listitem"
        ...
        >
          <button
            ...
            aria-label={`Arrastrar para reordenar "${SECTION_LABELS[key] || key}"`}
            className="cursor-grab active:cursor-grabbing text-stone-500 shrink-0"
            style={{ touchAction: "none" }}
          >
            <GripVertical size={15} aria-hidden="true" />
          </button>
          <span className="text-[12.5px] text-white flex-1">{SECTION_LABELS[key] || key}</span>
          <div className="flex items-center gap-0.5 shrink-0">
            <button
              onClick={() => moveByKeyboard(i, -1)}
              disabled={i === 0}
              aria-label={`Subir "${SECTION_LABELS[key] || key}"`}
              className="p-1 rounded text-stone-500 hover:text-white hover:bg-stone-700 transition disabled:opacity-25 disabled:hover:bg-transparent disabled:hover:text-stone-500"
            >
            ...
            <button
              ...
              aria-label={`Bajar "${SECTION_LABELS[key] || key}"`}
              className="p-1 rounded text-stone-500 hover:text-white hover:bg-stone-700 transition disabled:opacity-25 disabled:hover:bg-transparent disabled:hover:text-stone-500"
            >
...
const inputClass =
  "w-full bg-stone-800/60 border border-stone-700 rounded-md px-3 py-2 text-sm text-white placeholder-stone-500 focus:outline-none focus:ring-2 focus:ring-offset-0 focus:border-transparent transition";
...
function OnboardingTour({ open, onClose, accent }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-6 no-print">
      <div {...dialogProps} style={cardStyle} className="bg-stone-900 border border-stone-800 rounded-t-2xl sm:rounded-lg w-full max-w-sm p-6">
        {handle}
...
function useSwipeToDismiss(onClose, open) {
  const [dragY, setDragY] = useState(0);
  const draggingRef = useRef(false);
  const startYRef = useRef(0);

  // Cerrar con la tecla Escape, para quienes no usan mouse/dedo.
  useEffect(() => {
    if (!open) return;
    const onKeyDown = (e) => {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {columns.map((col) => (
            <div key={col.id}>
              ...
              <div
                className="border rounded-md overflow-hidden bg-stone-950 flex justify-center items-start py-3"
                style={{ borderColor: col.template === currentTemplate ? accent : "#3f3a35" }}
              >
                <div className="overflow-hidden shrink-0" style={{ width: 300, height: 480 }}>
                  <div style={{ width: 600, transform: "scale(0.5)", transformOrigin: "top left" }}>
...
function TemplateGallery({ open, onClose, current, accent, data, onSelect, onCompare, apiKey, currentPageCount }) {
  const { cardStyle, handle, dialogProps } = useSwipeToDismiss(onClose, open);
  ...
  // Antes de aplicar una plantilla, la medimos en un contenedor oculto para
  // avisar si haría que el CV pase a ocupar más páginas de las que ocupa hoy.
  const [pendingTemplate, setPendingTemplate] = useState(null);
  const measureRef = useRef(null);
  useEffect(() => {
    if (!pendingTemplate) return;
    const raf = requestAnimationFrame(() => {
      const node = measureRef.current?.querySelector(".print-page");
      let estimatedPages = 1;
      if (node) {
        const height = node.getBoundingClientRect().height;
        const corrected = height * 0.755;
        estimatedPages = Math.max(1, Math.ceil(corrected / 1123));
      }
      const templateId = pendingTemplate;
      setPendingTemplate(null);
      if (currentPageCount && estimatedPages > currentPageCount) {
        const ok = window.confirm(
          `Con esta plantilla tu CV pasaría de ${currentPageCount} a ${estimatedPages} página(s) aproximadamente (estimado). ¿Querés aplicarla igual?`
        );
        if (!ok) return;
      }
      onSelect(templateId);
  ...
  const PendingComponent = pendingTemplate ? TEMPLATE_COMPONENTS[pendingTemplate] : null;
        ...
        <div className="px-3 py-2.5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-white">{t.desc}</p>
            </div>
            {current === t.id && (
              <span
          ...
          {(TEMPLATE_FEATURES[t.id] || []).length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {TEMPLATE_FEATURES[t.id].map((f) => (
                <span
                  key={f}
                  className="text-[9px] px-1.5 py-0.5 rounded-sm bg-stone-800 text-white"
                >
                  {f}
                </span>
              ))}
    ...
    <div className="fixed inset-0 z-30 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-6 no-print">
      <div {...dialogProps} style={cardStyle} className="bg-stone-900 border border-stone-800 rounded-t-2xl sm:rounded-lg w-full max-w-2xl p-6 max-h-[85vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <button
              onClick={onCompare}
              className="text-[11px] font-mono uppercase tracking-wider text-white hover:text-white transition whitespace-nowrap"
            >
              Comparar
            </button>
            <button onClick={onClose} aria-label="Cerrar" className="text-white hover:text-white transition">
              <X size={20} aria-hidden="true" />
            </button>
          </div>
        </div>
        <div className="mb-5 p-3 rounded-md bg-stone-800/40 border border-stone-700">
          <p className="text-[10.5px] font-mono uppercase tracking-wider text-white mb-2">
          ...
          {suggestion && (
            <div className="mt-3 p-2.5 rounded-md bg-stone-950/60 border border-stone-700">
              <p className="text-[12.5px] text-white">
                ...
                {" — "}
                <span className="text-white">{suggestion.razon}</span>
              </p>
              <button
                onClick={() => setPendingTemplate(suggestion.template)}
        ...
        {favorites.length > 0 && (
          <div className="mb-6">
            <h3 className="text-[11px] font-mono uppercase tracking-widest text-white mb-3 flex items-center gap-2">
      ...
      {pendingTemplate && (
        <div
          ref={measureRef}
          aria-hidden="true"
          style={{ position: "fixed", top: -99999, left: -99999, width: 600, pointerEvents: "none" }}
        >
          {PendingComponent && <PendingComponent data={data} accent={accent} />}
        </div>
      )}
    </div>
  );
...
function CoverLetterModal({ open, onClose, data, accent, onUpdateField, onGenerate, generating }) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-6 no-print">
      <div {...dialogProps} style={cardStyle} className="bg-stone-900 border border-stone-800 rounded-t-2xl sm:rounded-lg w-full max-w-lg p-6 max-h-[85vh] overflow-y-auto">
        {handle}
        <div className="flex items-center justify-between mb-4">
...
function ImportModal({ open, onClose, onImport, importing, error }) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-6 no-print">
      <div {...dialogProps} style={cardStyle} className="bg-stone-900 border border-stone-800 rounded-t-2xl sm:rounded-lg w-full max-w-md p-6">
        {handle}
        <div className="flex items-center justify-between mb-4">
        ...
        {!result ? (
          <>
            <p className="text-[12.5px] text-stone-400 leading-relaxed mb-4">
              Subí un PDF, Word, o una foto/captura (JPG o PNG) de cualquier currículum (el tuyo, o de otra
              persona) y la IA lo revisa como lo haría un reclutador exigente, señalando errores concretos —
              sin crear ningún perfil nuevo ni tocar tus datos actuales.
            </p>
            <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-stone-700 rounded-lg py-8 cursor-pointer hover:border-stone-500 transition">
              <input
                type="file"
              ...
              <span className="text-[12px] text-stone-400">
                {analyzing ? "Analizando el archivo…" : "Tocá para elegir un archivo (PDF, Word, JPG o PNG)"}
              </span>
            </label>
            {error && <p className="text-[11px] text-red-400 mt-3 leading-relaxed">{error}</p>}
          </>
...
function PlainTextModal({ open, onClose, text, accent, fileName }) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-6 no-print">
      <div {...dialogProps} style={cardStyle} className="bg-stone-900 border border-stone-800 rounded-t-2xl sm:rounded-lg w-full max-w-lg p-6 max-h-[85vh] overflow-y-auto flex flex-col">
        {handle}
...
const PERSONAL_DATA_TIPS = [
  {
    tema: "Edad / fecha de nacimiento",
    texto:
      "En España y buena parte de Europa se está dejando de recomendar, porque facilita el descarte automático por edad en el primer filtro. En varios países de Latinoamérica todavía es común incluirla. No es ilegal ni incorrecto de ningún lado — es una decisión de estrategia según el país y el rubro.",
  },
  {
    tema: "Discapacidad",
    texto:
      "En la mayoría de los países no es obligatorio declararla, y discriminar por eso en la selección es ilegal. Puede convenir mencionarla si te postulás a un programa de inclusión laboral con cupo específico, o si vas a necesitar algún ajuste desde el primer día. Muchos especialistas recomiendan dejarla fuera del CV y hablarlo recién en la entrevista, para reducir el riesgo de sesgos inconscientes en el filtro inicial.",
  },
  {
    tema: "Estado civil",
    texto:
      "Cada vez menos frecuente a nivel internacional — la mayoría de las guías de RRHH actuales lo consideran información irrelevante para el desempeño en el puesto. Sigue siendo común en algunos países de Latinoamérica, pero es opcional en casi todos los casos.",
  },
  {
    tema: "Nacionalidad",
    texto:
      "Puede ser relevante si el puesto requiere un permiso de trabajo específico o si te postulás desde el exterior (aclara que no vas a tener trabas migratorias). Si sos del mismo país donde está el puesto, suele ser información redundante.",
  },
  {
    tema: "Foto",
    texto:
      "Común y hasta esperada en varios países de Latinoamérica y Europa continental. En cambio, en Estados Unidos, Reino Unido, Canadá y Australia se recomienda explícitamente NO incluirla, ya que puede generar problemas legales para quien contrata (riesgo de sesgo por apariencia, edad o género).",
  },
];

function PersonalDataTipsModal({ open, onClose, accent }) {
  const { cardStyle, handle, dialogProps } = useSwipeToDismiss(onClose, open);
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-6 no-print">
      <div {...dialogProps} style={cardStyle} className="bg-stone-900 border border-stone-800 rounded-t-2xl sm:rounded-lg w-full max-w-md p-6 max-h-[85vh] overflow-y-auto">
        {handle}
        <div className="flex items-center justify-between mb-2">
          <h2 className="font-display text-xl">Datos personales: ¿incluir o no?</h2>
          <button onClick={onClose} aria-label="Cerrar" className="text-stone-500 hover:text-white transition">
            <X size={20} aria-hidden="true" />
          </button>
        </div>
        <p className="text-[12px] text-stone-500 mb-4 leading-relaxed">
          Ninguno de estos datos es obligatorio en HazTuCV. Acá tenés un resumen para decidir vos qué
          conviene según tu país y a qué te postulás — no es un consejo legal, solo información general.
        </p>
        <ul className="flex flex-col gap-4">
          {PERSONAL_DATA_TIPS.map((item, i) => (
            <li key={i}>
              <p className="text-[13px] font-semibold mb-1" style={{ color: accent }}>
                {item.tema}
              </p>
              <p className="text-[12.5px] text-stone-300 leading-relaxed">{item.texto}</p>
            </li>
          ))}
        </ul>
        <p className="text-[11px] text-stone-600 mt-4 pt-3 border-t border-stone-800 leading-relaxed">
          Si después de leer esto querés incluir alguno de estos datos, podés hacerlo con total libertad
          desde la sección opcional "Información adicional" del formulario.
        </p>
      </div>
    </div>
  );
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-60 bg-stone-900 border border-stone-800 rounded-md shadow-xl z-40 py-1 no-print">
            <button
            ...
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-stone-200 hover:bg-stone-800 transition whitespace-nowrap disabled:opacity-50"
            >
            ...
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-red-400 hover:bg-stone-800 transition whitespace-nowrap"
            >
...
function Avatar({ data, accent, size = 64, shape = "circle", ring = false, className = "" }) {
  ...
  return (
    <img
      src={data.foto}
      alt={data.nombre ? `Foto de perfil de ${data.nombre}` : "Foto de perfil"}
...
function SkillChips({ data, accent }) {
  return (
    <p
      className="text-[11.5px] leading-relaxed px-2.5 py-1.5 rounded-sm break-words whitespace-pre-line"
      style={{ background: `${accent}18`, color: accent }}
    >
      {data.habilidades}
    </p>
      ...
      {items.map((idm) => (
        <div key={idm.id}>
          ...
          <div
            ...
            style={{ background: trackColor || "#e7e5e4" }}
            aria-hidden="true"
          >
            <div
...
function PreviewBandera({ data, accent }) {
  const hasExp = data.experiencia.some((e) => e.puesto || e.empresa);
  const hasEdu = data.educacion.some((e) => e.titulo || e.institucion);
  return (
    <div className="print-page bg-white text-[#232323] w-full max-w-[600px] shadow-2xl min-h-[849px] overflow-hidden">
      <div style={{ background: accent }} className="px-10 pt-10 pb-8">
        <div className="flex items-end gap-6">
          <div className="shrink-0 -mb-14">
            <Avatar data={data} accent="white" size={104} ring />
          </div>
          <div className="pb-1 min-w-0">
            <h1 className="font-display text-[26px] leading-[1.1] text-white uppercase font-bold break-words">
              {data.nombre || "Tu nombre"}
            </h1>
            {data.puesto && (
              <p className="text-[12px] font-bold uppercase tracking-wide mt-1.5" style={{ color: "#1a2f4a" }}>
                {data.puesto}
              </p>
            )}
          </div>
      ...
      <div className="grid grid-cols-2 gap-x-8 px-10 pt-16 pb-8">
        <div>
          <SectionLabel accent={accent} className="mb-3">Contacto</SectionLabel>
          <ContactLine data={data} className="flex flex-col gap-1.5 text-[11.5px] text-stone-600" iconSize={11} />
          {hasEdu && (
            <div className="mt-6">
              <SectionLabel accent={accent} className="mb-2">Educación</SectionLabel>
              <EducacionBlock data={data} />
        ...
        <div>
          {data.resumen && (
            <div className="mb-6">
              <SectionLabel accent={accent}>Acerca de mí</SectionLabel>
              <p className="text-[12px] leading-relaxed text-stone-700">{data.resumen}</p>
            </div>
          )}
          {data.habilidades && (
      ...
      {hasExp && (
        <div className="px-10 pb-10">
          <SectionLabel accent={accent} className="mb-3">Experiencia laboral</SectionLabel>
          <ExperienciaBlock data={data} accent={accent} titleClass="font-display text-[14px] font-medium text-stone-900" />
        </div>
      )}
    </div>
  );
...
function PreviewBanderaCentral({ data, accent }) {
  const hasExp = data.experiencia.some((e) => e.puesto || e.empresa);
  const hasEdu = data.educacion.some((e) => e.titulo || e.institucion);
  return (
    <div className="print-page bg-white text-[#232323] w-full max-w-[600px] shadow-2xl min-h-[849px] overflow-hidden">
      <div style={{ background: accent }} className="px-10 py-10 text-center">
        <div className="flex justify-center mb-4">
          <Avatar data={data} accent="white" size={100} ring />
        </div>
        <h1 className="font-display text-[26px] leading-[1.1] text-white uppercase font-bold break-words">
          {data.nombre || "Tu nombre"}
        </h1>
      ...
      <div className="px-10 pt-8">
        {data.resumen && (
          <p className="text-[12px] leading-relaxed text-stone-700 text-center max-w-[420px] mx-auto mb-8">
            {data.resumen}
          </p>
        )}
      </div>
      <div className="grid grid-cols-2 gap-x-8 px-10 pb-8">
        <div>
          <SectionLabel accent={accent} className="mb-3">Contacto</SectionLabel>
          <ContactLine data={data} className="flex flex-col gap-1.5 text-[11.5px] text-stone-600" iconSize={11} />
        </div>
...
function PreviewBanderaLinea({ data, accent }) {
  const hasExp = data.experiencia.some((e) => e.puesto || e.empresa);
  const hasEdu = data.educacion.some((e) => e.titulo || e.institucion);
  return (
    <div className="print-page bg-white text-[#232323] w-full max-w-[600px] shadow-2xl min-h-[849px] overflow-hidden">
      <div className="px-10 pt-10 pb-6">
        <div className="flex items-end gap-6">
          <div className="shrink-0">
            <Avatar data={data} accent={accent} size={92} ring />
          </div>
          <div className="pb-1 min-w-0">
            <h1 className="font-display text-[26px] leading-[1.1] uppercase font-bold break-words" style={{ color: "#1a2f4a" }}>
              {data.nombre || "Tu nombre"}
            </h1>
            {data.puesto && (
              <p className="text-[12px] font-bold uppercase tracking-wide mt-1.5" style={{ color: accent }}>
                {data.puesto}
              </p>
            )}
          </div>
      ...
      <div className="h-[3px] mx-10" style={{ background: accent }} />
      <div className="grid grid-cols-2 gap-x-8 px-10 pt-8 pb-8">
        <div>
          <SectionLabel accent={accent} className="mb-3">Contacto</SectionLabel>
          <ContactLine data={data} className="flex flex-col gap-1.5 text-[11.5px] text-stone-600" iconSize={11} />
          {hasEdu && (
...
function PreviewBarraLateral({ data, accent }) {
  const hasExp = data.experiencia.some((e) => e.puesto || e.empresa);
  const hasEdu = data.educacion.some((e) => e.titulo || e.institucion);
  return (
    <div className="print-page bg-white text-[#232323] w-full max-w-[600px] shadow-2xl min-h-[849px] flex overflow-hidden">
      <div className="w-[34%] bg-[#2b2b2e] px-6 py-8 flex flex-col gap-6 shrink-0">
        <div className="flex justify-center">
          <Avatar data={data} accent="white" size={88} ring />
        </div>
        <ContactLine
          data={data}
          className="flex flex-col items-center gap-1.5 text-[10.5px] text-white/80 text-center"
          iconSize={10}
        />
        {data.linkedin && (
          <div className="text-center">
            <p className="text-[9.5px] uppercase tracking-widest text-white font-bold mb-2 leading-snug">
              Sitios web, cartera de negocios, etc.
            </p>
            <p className="text-[10.5px] text-white/70 break-words">{data.linkedin}</p>
          </div>
        )}
        {hasEdu && (
          <div>
            <p className="text-[9.5px] uppercase tracking-widest text-white font-bold mb-2">
              Formación académica
            </p>
            <div className="flex flex-col gap-2.5">
              {data.educacion.map(
                (edu) =>
                  (edu.titulo || edu.institucion) && (
                    <div key={edu.id}>
                      <p className="text-[10.5px] text-white/70">{edu.titulo}</p>
                      <p className="text-[10.5px] text-white font-semibold">
                        {edu.institucion}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="px-8 py-8" style={{ background: accent }}>
          <h1 className="font-display text-[24px] leading-tight text-white uppercase font-bold break-words">
            {data.nombre || "Tu nombre"}
          </h1>
        </div>
        <div className="px-8 py-6 flex-1">
          {data.resumen && (
            <div className="mb-6">
              <SectionLabel accent={accent}>Perfil profesional</SectionLabel>
              <p className="text-[12px] leading-relaxed text-stone-700">{data.resumen}</p>
            </div>
          )}
          {hasExp && (
            <div className="mb-6">
              <SectionLabel accent={accent} className="mb-3">Experiencia laboral</SectionLabel>
              <ExperienciaBlock data={data} accent={accent} titleClass="font-display text-[13px] font-medium text-stone-900" />
            </div>
          )}
          {data.habilidades && (
            <div className="mb-6">
              <SectionLabel accent={accent} className="mb-2">Aptitudes</SectionLabel>
              <ul className="grid grid-cols-2 gap-x-4 gap-y-1">
                {data.habilidades.split(/[,\n]/).map(
                  (h, i) =>
                    h.trim() && (
                      <li key={i} className="text-[11.5px] text-stone-700 flex items-start gap-1.5">
          ...
          {data.seccionesOpcionales?.informacionAdicional && data.informacionAdicional && (
            <div>
              <SectionLabel accent={accent}>Información adicional</SectionLabel>
              <p className="text-[12px] leading-relaxed text-stone-700 whitespace-pre-line">
...
function PreviewPerfilBanda({ data, accent }) {
  const hasExp = data.experiencia.some((e) => e.puesto || e.empresa);
  const hasEdu = data.educacion.some((e) => e.titulo || e.institucion);
  return (
    <div className="print-page bg-white text-[#232323] w-full max-w-[600px] shadow-2xl min-h-[849px] overflow-hidden">
      <div style={{ background: accent }} className="px-8 pt-8 pb-6">
        <div className="flex items-start gap-5">
          <Avatar data={data} accent="white" size={90} ring />
          <div className="flex-1 pt-2 min-w-0">
            <h1 className="font-display text-[24px] leading-tight text-white uppercase font-bold break-words">
              {data.nombre || "Tu nombre"}
            </h1>
            {data.puesto && (
              <p className="text-white/90 text-[11.5px] font-bold uppercase tracking-wide mt-1">{data.puesto}</p>
            )}
          </div>
        </div>
        {data.resumen && (
          <div className="mt-5">
            <p className="text-white/90 text-[10px] uppercase tracking-[0.2em] font-bold mb-1.5">Perfil profesional</p>
            <p className="text-white/85 text-[11.5px] leading-relaxed">{data.resumen}</p>
          </div>
        )}
      </div>
      <div className="grid grid-cols-[38%_62%] gap-6 px-8 py-7">
        <div className="flex flex-col gap-6">
          <div>
            <SectionLabel accent={accent} className="mb-2">Contacto</SectionLabel>
            <ContactLine data={data} className="flex flex-col gap-1.5 text-[11px] text-stone-600" iconSize={10} />
          </div>
          {data.habilidades && (
            <div>
              <SectionLabel accent={accent} className="mb-2">Habilidades</SectionLabel>
        ...
        <div className="flex flex-col gap-6 min-w-0">
          {hasExp && (
            <div>
              <SectionLabel accent={accent} className="mb-3">Experiencia profesional</SectionLabel>
              <ExperienciaBlock data={data} accent={accent} titleClass="font-display text-[13px] font-medium text-stone-900" />
            </div>
          )}
          {hasEdu && (
            <div>
              <SectionLabel accent={accent} className="mb-2">Formación</SectionLabel>
              <EducacionBlock data={data} />
            </div>
          )}
        </div>
...
function PreviewSidebarDiagonal({ data, accent }) {
  const hasExp = data.experiencia.some((e) => e.puesto || e.empresa);
  const hasEdu = data.educacion.some((e) => e.titulo || e.institucion);
  return (
    <div className="print-page bg-white text-[#232323] w-full max-w-[600px] shadow-2xl min-h-[849px] overflow-hidden relative">
      <div
        className="absolute top-0 left-0 right-0 h-[110px]"
        style={{ background: "#1c2b4a", clipPath: "polygon(0 0, 100% 0, 100% 65%, 0 100%)" }}
      />
      <div className="relative px-8 pt-7 flex items-center gap-4">
        <Avatar data={data} accent="white" size={64} ring />
        <div className="min-w-0">
          <h1 className="font-display text-[20px] leading-tight text-white break-words">
            {data.nombre || "Tu nombre"}
          </h1>
          {data.puesto && <p className="text-[11px] mt-1 text-white/80 font-mono uppercase tracking-wide">{data.puesto}</p>}
        </div>
      </div>
      <div className="grid grid-cols-[36%_64%] gap-6 px-8 pt-10 pb-8">
        <div className="flex flex-col gap-6">
          <div>
            <SectionLabel accent={accent} className="mb-2">Contacto</SectionLabel>
            <ContactLine data={data} className="flex flex-col gap-1.5 text-[10.5px] text-stone-600" iconSize={10} />
          </div>
          {data.linkedin && (
            <div>
              <SectionLabel accent={accent} className="mb-2">Sitios web</SectionLabel>
            ...
            <div>
              ...
              <ul className="flex flex-col gap-1">
                {data.habilidades.split(/[,\n]/).map(
                  (h, i) =>
                    h.trim() && (
                      <li key={i} className="text-[10.5px] text-stone-700 flex items-start gap-1.5">
                        <span className="mt-1.5 w-1 h-1 rounded-full shrink-0" style={{ background: accent }} />
                        {h.trim()}
                      </li>
                    )
          ...
          {data.seccionesOpcionales?.informacionAdicional && data.informacionAdicional && (
            <div>
              <SectionLabel accent={accent} className="mb-2">Información adicional</SectionLabel>
              <p className="text-[10.5px] text-stone-700 leading-relaxed whitespace-pre-line">
                {data.informacionAdicional}
              </p>
            </div>
          )}
        ...
        <div className="flex flex-col gap-6 min-w-0">
          {data.resumen && (
            <div>
              <SectionLabel accent={accent} className="mb-2">Perfil</SectionLabel>
              <p className="text-[11.5px] leading-relaxed text-stone-700">{data.resumen}</p>
            </div>
          )}
          {hasExp && (
            <div>
...
function PreviewSidebarPerfil({ data, accent }) {
  const hasExp = data.experiencia.some((e) => e.puesto || e.empresa);
  const hasEdu = data.educacion.some((e) => e.titulo || e.institucion);
  return (
    <div className="print-page bg-white text-[#232323] w-full max-w-[600px] shadow-2xl min-h-[849px] flex overflow-hidden">
      <div className="w-[36%] px-6 py-8 flex flex-col gap-6 shrink-0" style={{ background: accent }}>
        <div className="flex justify-center">
          <Avatar data={data} accent="white" size={84} ring />
        </div>
        <div className="text-center">
          <h1 className="font-display text-[16px] leading-tight text-white break-words">
            {data.nombre || "Tu nombre"}
          </h1>
          {data.puesto && <p className="text-[10px] mt-1 text-white/80 font-mono uppercase">{data.puesto}</p>}
        </div>
        {data.resumen && (
          <div>
            <p className="text-[9.5px] uppercase tracking-widest text-white/80 font-bold mb-1.5">Perfil profesional</p>
            <p className="text-[10.5px] text-white/90 leading-relaxed">{data.resumen}</p>
          </div>
        )}
        <div>
          ...
          <div>
            ...
            <ul className="flex flex-col gap-1">
              {data.habilidades.split(/[,\n]/).map(
                (h, i) =>
                  h.trim() && (
                    <li key={i} className="text-[10.5px] text-white/90">
                      {h.trim()}
                    </li>
                  )
              )}
        ...
        {data.seccionesOpcionales?.informacionAdicional && data.informacionAdicional && (
          <div>
            <p className="text-[9.5px] uppercase tracking-widest text-white/80 font-bold mb-1.5">Información adicional</p>
            <p className="text-[10.5px] text-white/90 leading-relaxed whitespace-pre-line">
              {data.informacionAdicional}
            </p>
          </div>
        )}
      </div>
      <div className="flex-1 px-8 py-8 flex flex-col gap-6 min-w-0">
        {hasExp && (
          <div>
            <SectionLabel accent={accent} className="mb-3">Experiencia laboral</SectionLabel>
            <ExperienciaBlock data={data} accent={accent} titleClass="font-display text-[13px] font-medium text-stone-900" />
...
function PreviewBannerAncho({ data, accent }) {
  const hasExp = data.experiencia.some((e) => e.puesto || e.empresa);
  const hasEdu = data.educacion.some((e) => e.titulo || e.institucion);
  return (
    <div className="print-page bg-white text-[#232323] w-full max-w-[600px] shadow-2xl min-h-[849px] overflow-hidden">
      <div style={{ background: `${accent}22` }} className="px-8 py-6 flex items-center gap-5">
        <div className="flex-1 min-w-0">
          <h1 className="font-display text-[22px] leading-tight text-stone-900 break-words">
            {data.nombre || "Tu nombre"}
          </h1>
          {data.puesto && <p className="text-[12px] mt-1 font-semibold" style={{ color: accent }}>{data.puesto}</p>}
        </div>
        <Avatar data={data} accent={accent} size={76} ring shape="rounded" className="rounded-md" />
      </div>
      <div className="px-8 py-6">
        {data.resumen && (
          <div className="mb-5">
            <SectionLabel accent={accent} className="mb-1.5">Perfil</SectionLabel>
            <p className="text-[11.5px] leading-relaxed text-stone-700">{data.resumen}</p>
          </div>
        )}
        <ContactLine
        ...
        {data.seccionesOpcionales?.informacionAdicional && data.informacionAdicional && (
          <div>
            <SectionLabel accent={accent} className="mb-1.5">Información adicional</SectionLabel>
            <p className="text-[11.5px] leading-relaxed text-stone-700 whitespace-pre-line">
              {data.informacionAdicional}
            </p>
          </div>
        )}
...
function PreviewGeometrica({ data, accent }) {
  const hasExp = data.experiencia.some((e) => e.puesto || e.empresa);
  const hasEdu = data.educacion.some((e) => e.titulo || e.institucion);
  return (
    <div className="print-page bg-white text-[#232323] w-full max-w-[600px] shadow-2xl min-h-[849px] overflow-hidden">
      <div className="relative h-[150px] flex items-center justify-center overflow-hidden" style={{ background: `${accent}30` }}>
        <div
          className="absolute top-0 left-1/2 -translate-x-1/2 w-[220px] h-[150px]"
          style={{ background: accent, clipPath: "polygon(0 0, 100% 0, 50% 100%)" }}
        />
        <div className="relative z-10">
          <Avatar data={data} accent="white" size={110} ring />
        </div>
      </div>
      <div className="text-center px-8 pt-6 pb-6">
        <h1 className="font-display text-[26px] leading-tight font-bold text-stone-900 break-words">
          {data.nombre || "Tu nombre"}
        </h1>
        {data.puesto && <p className="text-[13px] text-stone-500 mt-1">{data.puesto}</p>}
      </div>
      <div className="grid grid-cols-2 gap-8 px-8 pb-8">
        <div>
          {hasExp && (
            <div>
              <h2 className="font-display font-bold text-[15px] text-stone-900 pb-1.5 mb-3 border-b border-stone-200">
                Experiencia laboral
              </h2>
              <ExperienciaBlock data={data} accent={accent} titleClass="font-display text-[13px] font-bold text-stone-900" />
            </div>
          )}
        </div>
        <div>
            ...
            <div className="mb-6">
              ...
              <ul className="flex flex-col gap-1.5">
                {data.habilidades.split(/[,\n]/).map(
                  (h, i) =>
                    h.trim() && (
                      <li key={i} className="text-[11.5px] text-stone-700">
                        {h.trim()}
                      </li>
                    )
                )}
...
function PreviewSerifDivisor({ data, accent }) {
  const hasExp = data.experiencia.some((e) => e.puesto || e.empresa);
  const hasEdu = data.educacion.some((e) => e.titulo || e.institucion);
  return (
    <div className="print-page bg-white text-[#232323] w-full max-w-[600px] shadow-2xl min-h-[849px] flex overflow-hidden">
      <div className="w-[32%] px-6 py-9 flex flex-col gap-6 shrink-0">
        <Avatar data={data} accent={accent} size={100} ring />
        <div>
          <p className="text-[10px] uppercase tracking-[0.2em] font-semibold text-stone-500 mb-2">
            Datos de contacto
          </p>
          <ContactLine data={data} className="flex flex-col gap-1.5 text-[11px] text-stone-700" iconSize={10} />
        </div>
        {hasEdu && (
          <div>
            <p className="text-[10px] uppercase tracking-[0.2em] font-semibold text-stone-500 mb-2">Educación</p>
            <EducacionBlock data={data} />
          </div>
        )}
        {data.seccionesOpcionales?.informacionAdicional && data.informacionAdicional && (
      ...
      <div className="w-px bg-stone-300 my-9" />
      <div className="flex-1 px-8 py-9 min-w-0">
        <h1 className="text-[34px] leading-[1.05] text-stone-900 break-words" style={{ fontFamily: "Georgia, serif" }}>
          {data.nombre || "Tu nombre"}
        </h1>
        {data.puesto && (
          <p className="text-[11px] uppercase tracking-[0.25em] font-semibold text-stone-600 mt-3">{data.puesto}</p>
        )}
        {data.resumen && <p className="text-[12px] leading-relaxed text-stone-700 mt-4">{data.resumen}</p>}
        {hasExp && (
          <div className="mt-7">
            <p className="text-[11px] uppercase tracking-[0.25em] font-semibold text-stone-600 mb-3">
              Experiencia laboral
            </p>
            <ExperienciaBlock data={data} accent={accent} titleClass="font-display text-[13px] font-semibold text-stone-900" />
          </div>
        )}
        {data.habilidades && (
          <div className="mt-6">
            <p className="text-[11px] uppercase tracking-[0.25em] font-semibold text-stone-600 mb-2">Habilidades</p>
...
function PreviewBandaCeleste({ data, accent }) {
  const hasExp = data.experiencia.some((e) => e.puesto || e.empresa);
  const hasEdu = data.educacion.some((e) => e.titulo || e.institucion);
  return (
    <div className="print-page bg-white text-[#232323] w-full max-w-[600px] shadow-2xl min-h-[849px] overflow-hidden">
      <div style={{ background: `${accent}30` }} className="px-8 pt-8 pb-6 flex items-center gap-5">
        <Avatar data={data} accent={accent} size={90} ring />
        <div className="min-w-0">
          <h1 className="font-display text-[26px] leading-tight font-bold break-words" style={{ color: accent }}>
            {data.nombre || "Tu nombre"}
          </h1>
          {data.puesto && (
            <p className="text-[11px] mt-1.5 font-bold uppercase tracking-wide" style={{ color: accent }}>
              {data.puesto}
            </p>
          )}
        </div>
      ...
      <div className="grid grid-cols-[36%_64%]">
        <div style={{ background: `${accent}10` }} className="px-6 py-7 flex flex-col gap-6">
          <div>
            <h2 className="text-[13px] font-bold uppercase tracking-wide mb-3" style={{ color: accent }}>
              Contacto
            </h2>
            <ContactLine data={data} className="flex flex-col gap-2 text-[11px] text-stone-700" iconSize={11} />
          </div>
          {data.habilidades && (
            <div>
              <h2 className="text-[13px] font-bold uppercase tracking-wide mb-2" style={{ color: accent }}>
                Habilidades
              </h2>
              <ul className="flex flex-col gap-1">
                {data.habilidades.split(/[,\n]/).map(
              ...
              <div className="flex flex-col gap-2.5">
                {data.educacion.map(
                  (edu) =>
                    (edu.titulo || edu.institucion) && (
                      <div key={edu.id}>
                        <p className="text-[11px] font-semibold text-stone-900">{edu.titulo}</p>
...
function PreviewSidebarReferencias({ data, accent }) {
  const hasExp = data.experiencia.some((e) => e.puesto || e.empresa);
  const hasEdu = data.educacion.some((e) => e.titulo || e.institucion);
  return (
    <div className="print-page bg-white text-[#232323] w-full max-w-[600px] shadow-2xl min-h-[849px] flex overflow-hidden">
      <div className="w-[32%] px-5 py-8 flex flex-col gap-6 shrink-0" style={{ background: accent }}>
        <div className="flex justify-center">
          <Avatar data={data} accent="white" size={80} ring />
        </div>
        <div>
        ...
        {data.seccionesOpcionales?.informacionAdicional && data.informacionAdicional && (
          <div>
            <p className="text-[10px] uppercase tracking-widest text-white/80 font-bold mb-1.5">
              Información adicional
            </p>
            <p className="text-[10.5px] text-white/90 leading-relaxed whitespace-pre-line">
              {data.informacionAdicional}
      ...
      <div className="flex-1 px-8 py-8 min-w-0">
        <h1 className="font-display text-[24px] leading-tight text-stone-900 break-words">
          {data.nombre || "Tu nombre"}
        </h1>
        {data.puesto && (
          <p className="text-[12px] font-bold uppercase tracking-wide mt-1" style={{ color: accent }}>
            {data.puesto}
          </p>
        )}
        {data.resumen && (
...
const TEMPLATE_COMPONENTS = {
  ...
  bandera: PreviewBandera,
  banderaCentral: PreviewBanderaCentral,
  banderaLinea: PreviewBanderaLinea,
  barraLateral: PreviewBarraLateral,
  perfilBanda: PreviewPerfilBanda,
  sidebarDiagonal: PreviewSidebarDiagonal,
  sidebarPerfil: PreviewSidebarPerfil,
  bannerAncho: PreviewBannerAncho,
  geometrica: PreviewGeometrica,
  serifDivisor: PreviewSerifDivisor,
  bandaCeleste: PreviewBandaCeleste,
  sidebarReferencias: PreviewSidebarReferencias,
};
...
"""${JSON.stringify(payload)}"""`;
  ...
  const [personalTipsOpen, setPersonalTipsOpen] = useState(false);
  ...
  const getShorteningTips = () => {
    ...
    if (getDensityValue(data.densidad) > 105) {
      tips.push(`Bajá la densidad de texto en Apariencia (tenés ${getDensityValue(data.densidad)}%) para ganar espacio sin borrar contenido.`);
    }
...
Si no encontrás problemas relevantes, devolvé {"errores": []}.`;
      ...
      if (isImage) {
        ...
        const prompt = `La imagen adjunta es una foto o captura de un currículum. ${analysisInstructions}`;
        result = await callClaude(prompt, { base64, mediaType });
      } else {
        const text = await extractTextFromFile(file);
        if (!text.trim()) throw new Error("No se encontró texto en el archivo.");
        const truncated = text.slice(0, 8000);
        const prompt = `A continuación está el texto extraído de un currículum. ${analysisInstructions}
...
"""${truncated}"""`;
      ...
      console.error(err);
      setAnalyzeError("No se pudo analizar el archivo. Probá con otro PDF, Word o imagen.");
    } finally {
      setAnalyzing(false);
      e.target.value = "";
    }
  ...
  const saveCustomColor = (color) => {
    setCustomColors((prev) => (prev.includes(color) ? prev : [...prev, color].slice(-8)));
  };
...
{"segundosLectura": <número entero>, "primerVistazo": "descripción breve de qué ve el reclutador en los primeros 6-7 segundos", "sugerencia": "sugerencia puntual, o un mensaje breve confirmando que ya está bien priorizado"}`;
  ...
  const buildFileName = () => {
    const combined = [data.nombre, data.puesto].filter(Boolean).join(" ").trim();
    if (!combined) return "CV";
    return combined.replace(/[\\/:*?"<>|]/g, "").replace(/\s+/g, "_");
  };
  ...
  const handlePrint = () => {
    ...
    window.print();
    // Restauramos el título original después de que se cierre el diálogo de
    // impresión, para no dejar el título de la pestaña cambiado permanentemente.
    setTimeout(() => {
  ...
  return (
    <div className={(tema === "claro" ? "light-mode " : "") + "min-h-screen bg-stone-950 text-stone-100 font-sans pb-20 lg:pb-0 print:min-h-0 print:pb-0 print:bg-white"}>
      ...
      <style>{`
        ...
        /* Para personas sensibles al movimiento (mareos, migrañas, trastornos
           vestibulares): si el sistema operativo tiene "reducir movimiento"
           activado, quitamos animaciones y transiciones no esenciales. */
        ...
        /* Foco visible y consistente para navegación por teclado, en toda la app. */
      ...
      <header className="no-print border-b border-stone-800 px-6 py-4 flex items-center justify-between bg-stone-950/95 backdrop-blur flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              onClick={() => saveCustomColor(data.accent)}
              title="Guardar este color en tu paleta"
              aria-label="Guardar este color en tu paleta"
              className="w-5 h-5 rounded-full border-2 border-dashed border-stone-600 hover:border-stone-400 flex items-center justify-center text-stone-400 hover:text-white transition shrink-0"
            >
              <Plus size={11} aria-hidden="true" />
            </button>
            {customColors.length > 0 && (
              <div className="flex items-center gap-1.5 pl-1.5 ml-0.5 border-l border-stone-700">
                {customColors.map((c) => (
                  <button
                    key={c}
                    onClick={() => {
                      if (data.accent === c) removeCustomColor(c);
      ...
      <div id="main-content" className="grid grid-cols-1 lg:grid-cols-2">
        ...
        >
          ...
          <section className="mb-8">
            ...
            <Field label="Densidad de texto">
              <div className="flex items-center gap-3">
                <input
                  ...
                  max={130}
                  step={1}
                  value={getDensityValue(data.densidad)}
                  onChange={(e) => update({ densidad: Number(e.target.value) })}
                  className="flex-1 h-1.5 rounded-full appearance-none cursor-pointer"
                  style={{
                    background: `linear-gradient(to right, ${accent} 0%, ${accent} ${((getDensityValue(data.densidad) - 70) / 60) * 100}%, #44403c ${((getDensityValue(data.densidad) - 70) / 60) * 100}%, #44403c 100%)`,
                  }}
                  aria-label="Densidad de texto"
                  aria-valuetext={`${getDensityValue(data.densidad)}%`}
                />
                <span className="text-[11px] font-mono text-white w-9 text-right shrink-0">
                  {getDensityValue(data.densidad)}%
                </span>
              </div>
              <div className="flex justify-between mt-1">
                <button
                  onClick={() => update({ densidad: 100 })}
                  className="text-[10px] text-white hover:text-white transition"
                >
                  ↺ Restablecer a 100%
                </button>
              </div>
            ...
            <Field label="Orden de las secciones">
              ...
              <button
                ...
                className="text-[10.5px] font-mono uppercase tracking-wider text-white hover:text-white transition mt-2 disabled:opacity-55 disabled:cursor-default disabled:hover:text-white"
              >
                ↺ Restablecer orden
              </button>
              ...
              <button
                ...
                className="text-[10.5px] font-mono uppercase tracking-wider text-white hover:text-white transition disabled:opacity-55 inline-flex items-center gap-1 mt-3"
              >
                {readingTimeLoading ? (
              ...
              {RESUMEN_LENGTHS.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className="text-[10.5px] px-2.5 py-1 rounded-full border border-stone-700 text-white hover:text-white hover:border-stone-500 transition disabled:opacity-55 inline-flex items-center gap-1"
                  >
              ...
              <div className="flex items-center gap-3">
                {data.experiencia.filter((e) => e.puesto || e.empresa).length >= 2 && (
                  <button
                    ...
                    className="text-[10.5px] font-mono uppercase tracking-wider text-white hover:text-white transition disabled:opacity-55 inline-flex items-center gap-1"
                  >
                    {gapCheckLoading ? (
                      <Loader2 size={11} className="animate-spin" aria-hidden="true" />
                    ) : (
            ...
            {gapResults !== null && (
              gapResults.length > 0 ? (
                <div className="mb-3 p-2.5 rounded-md bg-stone-800/50 border border-amber-500/30">
                  ...
                  <ul className="flex flex-col gap-1.5">
                    {gapResults.map((g, i) => (
                      <li key={i} className="text-[11.5px] text-white">
                        <span className="text-white">{g.entre}:</span> {g.detalle}
                      </li>
                    ))}
                  </ul>
                  <p className="text-[10px] text-white mt-2 leading-relaxed">
                    ...
                    <div className="pl-6 mt-2">
                      ...
                      {weakVerbsResults[exp.id] && (
                        weakVerbsResults[exp.id].length > 0 ? (
                          <div className="mt-2 p-2.5 rounded-md bg-stone-800/50 border border-stone-700">
                            <div className="flex items-center justify-between mb-1.5">
                              <p className="text-[10.5px] font-mono uppercase tracking-wider text-white">
          ...
          <div className="mt-2 mb-3">
            ...
            <button
              onClick={() => setPersonalTipsOpen(true)}
              className="mt-2 text-[10.5px] font-mono uppercase tracking-wider text-white hover:text-white transition inline-flex items-center gap-1"
            >
              <HelpCircle size={11} aria-hidden="true" /> ¿Incluyo edad, discapacidad, u otros datos personales?
            </button>
          </div>
          ...
          {data.seccionesOpcionales?.proyectos && (
            <section className="mt-6 mb-8">
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-mono text-[11px] uppercase tracking-widest text-white flex items-center gap-2">
                  <span className="w-4 h-px bg-stone-700" /> Proyectos <span className="text-white normal-case">· Desarrollador</span>
                </h2>
                <div className="flex items-center gap-3">
                  <button onClick={proyectosH.add} className="text-white hover:text-white transition" title="Agregar proyecto" aria-label="Agregar proyecto">
                    <Plus size={16} aria-hidden="true" />
                  </button>
                  <button
          ...
          {data.seccionesOpcionales?.publicaciones && (
            <section className="mb-8">
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-mono text-[11px] uppercase tracking-widest text-white flex items-center gap-2">
                  <span className="w-4 h-px bg-stone-700" /> Publicaciones <span className="text-white normal-case">· Académico</span>
                </h2>
                <div className="flex items-center gap-3">
                  <button onClick={publicacionesH.add} className="text-white hover:text-white transition" title="Agregar publicación" aria-label="Agregar publicación">
                    <Plus size={16} aria-hidden="true" />
                  </button>
                  <button
          ...
          {data.seccionesOpcionales?.becas && (
            <section className="mb-8">
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-mono text-[11px] uppercase tracking-widest text-white flex items-center gap-2">
                  <span className="w-4 h-px bg-stone-700" /> Becas y reconocimientos <span className="text-white normal-case">· Académico</span>
                </h2>
                <div className="flex items-center gap-3">
                  <button onClick={becasH.add} className="text-white hover:text-white transition" title="Agregar beca" aria-label="Agregar beca">
                    <Plus size={16} aria-hidden="true" />
                  </button>
                  <button
          ...
          {data.seccionesOpcionales?.logros && (
            <section>
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-mono text-[11px] uppercase tracking-widest text-white flex items-center gap-2">
                  <span className="w-4 h-px bg-stone-700" /> Logros destacados <span className="text-white normal-case">· Ventas / Marketing</span>
                </h2>
                <div className="flex items-center gap-3">
                  <button onClick={logrosH.add} className="text-white hover:text-white transition" title="Agregar logro" aria-label="Agregar logro">
                    <Plus size={16} aria-hidden="true" />
                  </button>
                  <button
        ...
        <div className="bg-stone-900 px-6 py-8 flex justify-center print:bg-white print:p-0">
          <div
            ...
            style={{ zoom: (getDensityValue(data.densidad) / 100) }}
          >
            <div className="relative w-full max-w-[600px]">
              ...
              {pageCount > 1 && (
                <div
                  ...
                  style={{ top: PAGE_HEIGHT_PX / PRINT_WIDTH_CORRECTION / (getDensityValue(data.densidad) / 100) }}
                  aria-hidden="true"
                >
      ...
      {showBackToTop && (
        <button
          ...
          style={{ background: accent }}
        >
          <ArrowUp size={18} className="text-stone-950" aria-hidden="true" />
        </button>
      )}
